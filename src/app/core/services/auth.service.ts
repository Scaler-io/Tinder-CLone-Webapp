...
export class AuthService {
  ...
  public login(loginRequest): Observable<AuthUser> {
    return this.http
      .post<AuthUser>(
        `${environment.tinderCloneApiUrl}/account/login`,
        loginRequest
      )
      .pipe(
        map((response) => {
  ...
  public performAutoLogin(): Observable<AuthUser> {
    ...
    if (token) {
      return this.http
        .get<AuthUser>(`${environment.tinderCloneApiUrl}/account`)
        .pipe(
          map((response) => {
            return response;
          })
  ...
  public checkUserNameExists(username: string): Observable<boolean> {
    return this.http
      .get<boolean>(
        `${environment.tinderCloneApiUrl}/account/IsUsernameExists?username=${username}`
      )
      .pipe(
        map((response) => {
          return response;
  ...
  public isUserIsASystemAdmin(user: AuthUser): boolean {
    if (user.roles.length == 1 && user.roles.includes(UserRoles.Administrator))
      return true;
    return false;
  }

  public isUserIsAModerator(user: AuthUser): boolean {
    if (user.roles.length == 1 && user.roles.includes(UserRoles.Moderator))
      return true;
    return false;
  }

  public isUserIsAMember(user: AuthUser): boolean {
    if (user.roles.length == 1 && user.roles.includes(UserRoles.Member))
      return true;
    return false;
  }
}
